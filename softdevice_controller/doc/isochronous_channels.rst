...
BIS Parameter Evaluation
   The tool provided below enables you to explore how various ISO parameter settings impact the transport latency and synchronization delay of the stream created.
   ...
   .. note::
      * The calculated values of the sync delay and the transport latency are approximations and the actual values may differ depending on the platform.
      * Event layout is only shown for a single BIS.
      * Input parameters are not validated. Refer to `Bluetooth Core Specification`_ for requirements on the ISO parameters that make up a valid configuration.
