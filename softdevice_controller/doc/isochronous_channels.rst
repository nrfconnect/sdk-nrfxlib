...
.. note::
   LE Isochronous Channels channels and the corresponding documentation are currently :ref:`experimental <nrf:software_maturity>` and subject to changes.

LE Isochronous Channels channels is a feature defined in the `Bluetooth Core Specification`_.
It allows for unreliable transport of data in one-to-one, one-to-many and many-to-one topologies.

In LE Isochronous Channels channels, data is transmitted in units called SDUs.
...
The LE Isochronous Channels channels feature is split into a broadcast and a unicast part.
...
The LE Isochronous Channels channels feature is highly configurable and it is not feasible to test all possible configurations and topologies.
Testing of the |controller| implementation of LE Isochronous Channels channels focuses on assumed common use-cases.
...
Timestamps
   ...
   The timestamp of a previous SDU can be retrieved using the LE Read ISO TX sync command.
   ...
   When running the CIS peripheral, the LE Read ISO TX sync needs to be called periodically, and should be called every time before new data is provided.
...
Sequence numbers
   ...
   The controller learns the initial sequence number, so there is no need to align the sequence number each time with the one that is returned when calling the LE Read ISO TX sync command.
   ...
   To do this, use the LE Read ISO TX sync command.
