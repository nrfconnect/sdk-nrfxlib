...
.. list-table:: Operation latency
   :header-rows: 1
   :widths: auto

   * - Timing symbol
     - API Parameter
   * - :math:`t_{sched\_opr\_adv}`
     - ``scheduled_operation_advance``
   * - :math:`t_{idle\_to\_active}`
     - ``idle_to_active``
   * - :math:`t_{sched\_opr\_adv}` + :math:`t_{idle\_to\_active}`
     - ``scheduled_operation_advance`` + ``idle_to_active``
   * - :math:`t_{min\_sched\_opr\_trans\_dly}`
     - ``minimum_scheduled_operation_transition_delay``
   * - :math:`t_{active\_to\_idle}`
     - ``active_to_idle``

The following lists the timing symbols of configuration duration and their corresponding API parameters:

.. list-table:: Configuration duration
   :header-rows: 1
   :widths: auto

   * - Timing symbol
     - API Parameter
   * - :math:`t_\text{activation}`
     - ``activation``
   * - :math:`t_\text{deactivation}`
     - ``deactivation``
   * - :math:`t_\text{configuration}`
     - ``configuration``
   * - :math:`t_\text{initialization}`
     - ``initialization``
   * - :math:`t_\text{deinitialization}`
     - ``deinitialization``
...
Thus, the operation must be scheduled at the time as set in :math:`t_{sched\_opr\_adv}` + :math:`t_{idle\_to\_active}` in advance to ensure that the on‑air activity begins exactly at the desired time.
In this context, ``rssi`` operations are executed in the RX state, as shown in the figure.

You must ensure that the time between any two scheduled operations is not shorter than the value specified by :math:`t_{min\_sched\_opr\_trans\_dly}`.
...
The possible ``SLEEP`` wake-up times are factored into :math:`t_{sched\_opr\_adv}`.
