...
uint8_t rc_temp_ctiv;           /**< Only for ::MPSL_CLOCK_LF_SRC_RC: How often (in number of calibration
  ...
  bool skip_wait_lfclk_started; /**< Determines whether MPSL waits for the low frequency clock to start during initialization or not.
                                     If it is set to true, MPSL will wait for the low frequency clock later,
                                     before the low frequency clock is used for the first time. Default value is @ref MPSL_DEFAULT_SKIP_WAIT_LFCLK_STARTED. */
