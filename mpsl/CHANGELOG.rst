...
* Fixed an issue where a bug in mpsl could make higher layers assert (DRGN-23412).
...
* Added the :c:func:`mpsl_relaxed_real_time_set` function to enable or disable relaxed real-time mode. (DRGN-27671)
  When relaxed real-time mode is enabled, MPSL handles missed real-time deadlines gracefully instead of asserting.
