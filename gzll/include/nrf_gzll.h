...
typedef enum
{
    NRF_GZLL_TX_POWER_8_DBM,   ///<  8 dBm transmit power.
    NRF_GZLL_TX_POWER_7_DBM,   ///<  7 dBm transmit power.
    NRF_GZLL_TX_POWER_6_DBM,   ///<  6 dBm transmit power.
    NRF_GZLL_TX_POWER_5_DBM,   ///<  5 dBm transmit power.
    NRF_GZLL_TX_POWER_4_DBM,   ///<  4 dBm transmit power.
    NRF_GZLL_TX_POWER_3_DBM,   ///<  3 dBm transmit power.
    NRF_GZLL_TX_POWER_2_DBM,   ///<  2 dBm transmit power.
...
#ifdef NRF51
    NRF_GZLL_TX_POWER_N30_DBM  ///< -30 dBm transmit power.
#else
    NRF_GZLL_TX_POWER_N40_DBM  ///< -40 dBm transmit power.
#endif
} nrf_gzll_tx_power_t;
...
typedef enum
{
    ...
    NRF_GZLL_DATARATE_1MBIT_BLE = 3, ///<  1 Mbps datarate. BLE.
    NRF_GZLL_DATARATE_2MBIT_BLE = 4  ///<  2 Mbps datarate. BLE.
} nrf_gzll_datarate_t;

/**
 * @enum nrf_gzll_xosc_ctl_t
 * @brief Enumerator used for specifying whether switching the
 * external high frequency oscillator on/off shall be handled automatically
...
/**
 * @brief Set a timeslot tick timer.
 *
 * @param num_ticks Number of timeslot ticks.
 * @param callback  Callback function when timer expires.
 *
 * @retval true  If the timer was set.
 * @retval false If the number of ticks is not zero but the callback was NULL.
 */
