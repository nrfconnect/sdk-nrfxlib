...
static bool nrf_rpc_request_completion(bool *busy)
{
	bool request_done;
	...
	if (request_done) {
		...
		*busy = (processing_in_progress_count > 0);
	}
	nrf_rpc_os_mutex_unlock(&rpc_state_mutex);
...
int nrf_rpc_stop(bool cleanup)
{
	int err;
	bool busy = false;
	bool requested = nrf_rpc_request_completion(&busy);
	if (requested) {
		err = busy ? nrf_rpc_os_event_wait(&processing_complete_event,
						   CONFIG_NRF_RPC_STOP_WAIT_TIME) : 0;
		if (err) {
