			      ...
			      const uint8_t *packet, size_t len)
{
	...
	*group = group_from_strid(init_data.strid, init_data.strid_len);
	...
	/*
	 * Mark the transport as initialized in case `nrf_rpc_init` is preempted before doing this and
	 * then an init packet is received. Without this, sending a response to this packet would fail.
	 */
			    ...
			    void *context)
{
	...
	if (hdr.type == NRF_RPC_PACKET_TYPE_CMD ||
		...
		/*
		 * Mark the transport as initialized in case `nrf_rpc_init` is preempted before doing this and
		 * then a packet is received. Without this line, sending a response to this packet would fail.
		 */
		group->data->transport_initialized = true;
	}
